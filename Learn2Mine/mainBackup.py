...
class HomeHandler(webapp2.RequestHandler):
	...
	'''try:
		badgeRequest = getBadges.main(useremail)
		badgeID = ast.literal_eval(badgeRequest[1])['userId']
		badgesInfo = urllib2.urlopen('https://beta.openbadges.org/displayer/'+str(badgeID)+'/groups.json').read()
		groups = ast.literal_eval(badgesInfo)['groups']

		for group in groups:
			if(group['name'].lower() == 'learn2mine'):
				groupID = group['groupId']

		template_values = {'badgeID': badgeID, 'groupID': groupID, 'user':useremail}
	except:
...
class LeaderboardHandler(webapp2.RequestHandler):
	...
	#If no request, generate general welcome to Leaderboard message
		...
		self.response.write('<div class="lesson"><div class="lessontext"><div style="padding: 20px; padding-right:20%;"><h3>Welcome to the Leaderboards!<br /> Here, you can see how your skills stack up against other Learn2Mine users.<br/><br/> Click a lesson from the list on the right to view the top 10 scoring users for that lesson.</div></div>')
