...
def strategyA(fullCounts):
    max = 0
    index = 0
    for i in range(3):
        comparison = fullCounts[i]
        if max < comparison:
            max = comparison
            index = i
    return index

def strategyB(fullCounts, lastTen):
    max = 0
    index = 0
    for i in range(3):
        comparison = (.2 * fullCounts[i]) + (.8 * lastTen[i])
        if max < comparison:
            max = comparison
            index = i
    return index
...
if numGames == 0: # first game can't track score yet
    ...
    output = randChoice()
    myHistory.append(output)
elif numGames < 20: # first 10 games tracking score and counts but still random
    score = calcScore(myHistory[-1], opHistory[-1], score)
    historyTracker(myHistory[-2],opHistory[-1])
    myLastMove = myHistory[-1]
    if myLastMove == 'R':
        index = strategyA(rockCounts)
    elif myLastMove == 'P':
        index = strategyA(paperCounts)
    else:
        index = strategyA(scissorCounts)
    ...
    if previousResult() == 'L' and wins < losses: # opponents figuring you out, get it together
        offset = (offset + 1) % 3
    lastTenTracker(myHistory[-21:-1],opHistory[-20:])
    myLastMove = myHistory[-1]
    if myLastMove == 'R':
        index = strategyB(rockCounts, rockTen)
    elif myLastMove == 'P':
        index = strategyB(paperCounts, paperTen)
    else:
        index = strategyB(scissorCounts, scissorTen)
    output = rps[(index + 1 + offset) % 3]