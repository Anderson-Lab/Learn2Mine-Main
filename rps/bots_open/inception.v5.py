...
def previousResults(num):
    score = 0
    for i in range(num):
        opPlay = opHistory[-1 * (num+i)]
        myPlay = myHistory[-1 * (num+i)]
        mScore = rps.index(myPlay)
        oScore = rps.index(opPlay)
        if (mScore + 1) % 3 == oScore:
            score -= 1
...
if numGames == 0: # first game can't track score yet
    ...
    if previousResults(5) < 2 and wins < losses: # opponents figuring you out, get it together
        throwOff = True
