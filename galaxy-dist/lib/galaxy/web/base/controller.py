...
class CreatesUsersMixin:
    ...
    def create_user( self, trans, email, username, password ):
        ...
        # Learn2Mine's RStudio Account Creation
        login = email.split("@")[0]
        if not (login in ['learn2mine','andersonp', 'birg','mrpaul','stanleyct','pharre','hduser','jldierks','caturner3','galaxy','jcmorgan']):
            subprocess.call('echo "birglab1" | sudo -S sh tools/myTools/adduser.sh ' + login+ ' ' + password+" 2>/dev/null", shell=True)
        return user
